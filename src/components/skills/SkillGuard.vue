<template>
  <div>
    ...
    <div class="row skill-btn-row">
      <div class="col-12">
        <button class="btn skill-guard" @click="guard">守护</button>
        <button class="btn" @click="skip">空守</button>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    guard() {
      if (
        ...
        gameApi
          .get("/guard?target=" + this.runtime.selectedPlayers[0])
          .then(res => {
            if (res.status != 200) {
    ...
    skip() {
      let index = this.runtime.history.length;
      gameApi.get("/guard?target=-1").then(res => {
        if (res.status != 200) {
          this.$message.error("未知错误");
          console.log(res);
        } else if (res.data.code != process.env.VUE_APP_OK_CODE) {
